...
def _real_user_homes():
    """
    Every real user's home directory, from /etc/passwd - the agent runs as
    root with no session of its own, so this is the only way to find where
    a user's AI-CLI tools might have installed skills.
    """
    homes = []
    for line in (_read("/etc/passwd") or "").splitlines():
        f = line.split(":")
        if len(f) < 7:
            continue
        uid, home = f[2], f[5]
        if uid.isdigit() and int(uid) >= 1000 and os.path.isdir(home):
            homes.append(home)
    return homes
...
def _find_ai_skills():
    ...
    directory found across every real user's home."""
    found = []
...
def check_ai_skills():
    ...
    for tool, base, skills in dirs:
        for name in skills:
            ...
            out = _run([scanner, "scan", os.path.join(base, name), "--no-llm", "--format", "json"],
                       timeout=20)
            try:
